...
void gamePaint()
{
	...
	TCHAR scoreText[50];
	_stprintf(scoreText, _T("分数: %d"), score);
	outtextxy(10, 10, scoreText);       // 在(10,1-)位置输出分数

	// 绘制分数背景框（增加可读性）
	setfillcolor(RGB(30, 30, 30));  // 半透明黑色背景
	setfillstyle(BS_SOLID);
	fillrectangle(5, 5, 90, 35);    // 分数背景框

	// 绘制分数文本（带阴影效果）
	settextcolor(GREEN);             // 先绘制阴影
	outtextxy(11, 11, scoreText);   // 阴影位置偏移1像素
	settextcolor(WHITE);            // 再绘制白色文本
	outtextxy(10, 10, scoreText);   // 实际文本位置
...
void gameInterval()
{
	...
	if (appleHang == snakeHang[0] && appleLie == snakeLie[0] /*todo:  能吃 苹果*/)
	{
		score++;
		//todo:  变长
		snakeLen++;
		...
		while (1)
		{
			...
			for (int i = 0; i < snakeLen; i++) {
				if (appleHang == snakeHang[i] && appleLie == snakeLie[i])
				{
					flag = 0;
				}
				if (flag == 0)break;
	...
	if (snakeHang[0] == 0 || snakeHang[0] == 19 || snakeLie[0] == 0 || snakeLie[0] == 19/*todo:  能 出界*/)
	{
		//todo: gameover
		stop = 1;
	}
